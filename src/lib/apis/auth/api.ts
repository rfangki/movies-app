...
export const postLogin = async (body: FormSchema) => {
  try {
    const response = await axiosWithConfig.post(`authentication/token/validate_with_login`, body);
...
export const createSessionID = async (request_token: string) => {
  ...
  try {
    const response = await axiosWithConfig.post(`/authentication/session/new`, body);
...
export const getDetailAccount = async (session_id: string) => {
  try {
    const response = await axiosWithConfig.get(`/account?session_id=${session_id}`);
